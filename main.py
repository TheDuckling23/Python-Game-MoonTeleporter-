...
pygame.display.set_caption('Moon Teleporter')
fps = 60
...
def draw_inventory():
    ...
    time_text2 = font.render(f'{time//35} seconds', True, 'white')
    screen.blit(time_text, (600, HEIGHT - 110))
    screen.blit(time_text2, (600, HEIGHT - 80))
    best_text = font.render(f'Best Time: {high_score} s', True, 'light gray')
    screen.blit(best_text, (600, HEIGHT - 50 ))
...
def print_end(win_or_lose):
    ...
    win_text2 = font.render(f'Your Time: {time//35} s', True, 'white')
...
while run:
    timer.tick(fps)
    font = pygame.font.Font('freesansbold.ttf', 20)
    real_runtime = timer.get_fps()
    actual_fps = font.render(str(round(real_runtime, 2)), True, 'white')
    screen.blit(actual_fps, (10, 10))
    if counter < 19:
        counter += 1
    else:
        counter = 0
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if not win and not lose:
                ...
                if win and time//35 < high_score or high_score == 0 or high_score == '':
